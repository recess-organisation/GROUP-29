...
async function canAccessCourse(user, course) {
  if (user.role === 'admin' || canManageCourse(user, course)) return true;
  if (user.role === 'student') return studentIsEnrolled(user.id, course.id);
  return false;
}
...
async function createLesson(req, res) {
  try {
    ...
    if (!course_id || !title) return res.status(400).json({ message: 'Course and lesson title are required.' });
    const course = await findCourse(course_id);
    if (!course) return res.status(404).json({ message: 'Course not found.' });
    if (!canManageCourse(req.user, course)) return res.status(403).json({ message: 'You can only add lessons to your own courses.' });
...
async function getLessonsByCourse(req, res) {
  try {
    ...
    if (!course) return res.status(404).json({ message: 'Course not found.' });
    if (!(await canAccessCourse(req.user, course))) return res.status(403).json({ message: 'Enroll in this course to view lessons.' });
...
async function getLessonById(req, res) {
  try {
    ...
    if (!(await canAccessCourse(req.user, course))) return res.status(403).json({ message: 'You cannot view this lesson.' });
...
async function updateLesson(req, res) {
  try {
    ...
    if (lessons.length === 0) return res.status(404).json({ message: 'Lesson not found.' });
    const course = await findCourse(lessons[0].course_id);
    if (!canManageCourse(req.user, course)) return res.status(403).json({ message: 'You can only edit lessons for your own courses.' });
...
async function deleteLesson(req, res) {
  try {
    ...
    if (lessons.length === 0) return res.status(404).json({ message: 'Lesson not found.' });
    const course = await findCourse(lessons[0].course_id);
    if (!canManageCourse(req.user, course)) return res.status(403).json({ message: 'You can only delete lessons for your own courses.' });
...
module.exports = { createLesson, getLessonsByCourse, getLessonById, updateLesson, deleteLesson, studentIsEnrolled, canAccessCourse };
