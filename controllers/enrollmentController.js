...
async function enrollInCourse(req, res) {
  try {
    ...
    if (!course || course.status !== 'active') return res.status(404).json({ message: 'Active course not found.' });
    const existing = await db.query('SELECT id FROM enrollments WHERE student_id=? AND course_id=?', [req.user.id, req.params.courseId]);
    if (existing.length > 0) return res.status(409).json({ message: 'You are already enrolled in this course.' });
    const result = await db.query('INSERT INTO enrollments (student_id, course_id) VALUES (?, ?)', [req.user.id, req.params.courseId]);
...
async function getMyEnrolledCourses(req, res) {
  try {
    const courses = await db.query(
      `SELECT e.id AS enrollment_id, e.progress_percentage, e.points, e.streak, e.status AS enrollment_status,
        c.*, u.full_name AS teacher_name, t.name AS tier_name, s.name AS subject_name
       FROM enrollments e JOIN courses c ON c.id=e.course_id JOIN users u ON u.id=c.teacher_id
       LEFT JOIN learning_tiers t ON t.id=c.tier_id LEFT JOIN subjects s ON s.id=c.subject_id
...
async function getStudentsInCourse(req, res) {
  try {
    ...
    if (!canManageCourse(req.user, course)) return res.status(403).json({ message: 'You can only view students in your own courses.' });
    const students = await db.query(
      `SELECT e.id AS enrollment_id, e.progress_percentage, e.points, e.streak, e.status AS enrollment_status,
        u.id, u.full_name, u.email, u.phone, e.enrolled_at
       FROM enrollments e JOIN users u ON u.id=e.student_id
       WHERE e.course_id=? ORDER BY e.enrolled_at DESC`, [req.params.courseId]
    );
...
async function updateProgress(req, res) {
  try {
    ...
    if (enrollments.length === 0) return res.status(404).json({ message: 'Enrollment not found.' });
...
module.exports = { enrollInCourse, getMyEnrolledCourses, getStudentsInCourse, updateProgress };
